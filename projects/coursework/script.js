...
const saveState = () => {
    const data = {      //Создает объект с данными котрые нужно сохранить
...
const loadState = () => {  // функцию загрузки сохраненного состояния
    ...
    try {  // Начинает блок, где возможна ошибка при разборе JSON
...
const loadParticipants = () => {
    ...
    r.onload = () => {
        try {   // блок обработки, где могут возникнуть ошибки парсинга
            ...
            history = []; // очищает старую историю для нового списка участников
            saveState();   // сохраняет новое состояние приложения

            renderParticipants()  // перерисовывает список участников на странице
            renderHistory()   // перерисовывает историю на странице
...
const startSpin = () => {
    ...
    const animate = (time) => {
        const part = Math.min((time - startTime) / allTime, 1) // считает прогресс анимации от 0 до 1

        const nicePart = easeOutCubic(part);

        const deg = start + end * nicePart; // вычисляет текущий угол колеса
...
const finishDraw = (num) => {
    ...
    const gift = prizes[num]; // берет приз по выбранному номеру сектора
...
const renderParticipants = () => {
    ...
    participants.forEach((person) => {  // перебирает всех участников
...
const renderHistory = () => {
    // история перерисовывается полностью
...
const downloadHistory = () => {
    ...
    const txt = JSON.stringify(history, null, 4); // превращает историю в JSON

    const file = new Blob([txt], { type: "application/json" });

    const a = document.createElement("a"); // создает временную ссылку для скачивания.

    a.href = URL.createObjectURL(file); 

    a.download = "history.json"; // задает имя скачиваемого файла.
    ...
    URL.revokeObjectURL(a.href); // Освобождает временный URL из памяти.
} // Завершает функцию downloadHistory.
